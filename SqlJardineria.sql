...
--Command used to delete the database if it exists
...
--Command to use the database so you can run queries on it

USE db_jardineria;
go

-- Este comando sirve para crear la tabla oficina
-- This command is used to create the office table
...
-- Este comando sirve para crear la tabla empleado
-- This command is used to create the employee table
...
-- Este comando sirve para crear la tabla gama producto
-- This command is used to create the gama_product table
...
-- Este comando sirve para crear la tabla cliente
-- This command is used to create the customer table
...
-- Este comando sirve para crear la tabla pedido
-- This command is used to create the order table
CREATE TABLE pedido (
...
-- Este comando sirve para crear la tabla producto
-- This command is used to create the product table
...
-- Este comando sirve para crear la tabla detalle_pedido
-- This command is used to create the order_detail table
...
-- Este comando sirve para crear la tabla pago
-- This command is used to create the payment table
CREATE TABLE pago (
  codigo_cliente INTEGER NOT NULL,
...
-- Ejemplo de datos insertados en la tabla oficina
-- Example of data inserted into the office table
...
-- Ejemplo de datos insertados en la tabla empleado
-- Example of data inserted into the employee table
...
-- Ejemplo de datos insertados en la tabla gama_producto
-- Example of data inserted into the gama_product table
...
-- Ejemplo de datos insertados en la tabla cliente
-- Example of data inserted into the customer table
...
-- Ejemplo de datos insertados en la tabla pedido
-- Example of data inserted into the order table
...
-- Ejemplo de datos insertados en la tabla producto
-- Example of data inserted into the product table
...
-- Ejemplo de datos insertados en la tabla detalle_pedido
-- Example of data inserted into the order_details table
...
-- Ejemplo de datos insertados en la tabla pago
-- Example of data inserted into the office payment
...
--This command display all data from the customers table
select * from cliente

--Este comando muestra todos los datos de la tabla empleado
--This command display all data from the employee table
select * from empleado

--Este comando muestra todos los datos de la tabla gama_producto
--This command displays all the data in the gama_product table
select * from gama_producto

--Este comando muestra todos los datos de la tabla ofcina
--This command display all data from the office table
select * from oficina

--Este comando muestra todos los datos de la tabla producto
--This command display all data from the product table
select * from producto

--Este comando muestra todos los datos de la tabla pago
--This command display all data from the payment table
select * from pago

--Este comando muestra todos los datos de la tabla pedido
--This command display all data from the order tale
select * from pedido

--Este comando muestra todos los datos de la tabla detalle_pedido
--This command display all data from the order_details
select * from detalle_pedido

--evuelve un listado con el codigo de oficina y la ciudad donde hay oficinas
select o.codigo_oficina, o.ciudad 
from oficina o

--Seleccionar la ciudad y el telefono de las oficinas que estan en España
select o. ciudad, o.telefono 
from oficina o
where o.pais='España'

--Seleccionar el nombre, los apellidos y el email de los empleados que tienen el codigo de jefe 7
select e.nombre, e.apellido1, e.apellido2, e.email from empleado e
where e.codigo_jefe=7

--Seleccionar el puesto, el nombre, los apellidos y el email del hefe de la empresa
...
--Ejemplo para buscar un nombre diferente diferentes--
...
select c.nombre_contacto from cliente c
where c.nombre_cliente<>'Lasas S.A.'

select c.nombre_contacto from cliente c
where c.nombre_cliente='Lasas S.A.'

-- % buscar un valor que empieze con Luis
select c.nombre_contacto 
from cliente c
where c.nombre_contacto
like 'luis%'

--% buscar un valor de los que terminen en an
select c.nombre_contacto 
from cliente c
where c.nombre_contacto
like '%an'

--Seleccionar el nombre, apellido y telefono de los clientes que empiezan con ju
select c.nombre_contacto, c.apellido_contacto, c.telefono  
from cliente c
where c.nombre_contacto
like 'ju%'

--Seleccionar el nombre, apellido y telefono de los clientes que terminan con a
select c.nombre_contacto, c.apellido_contacto, c.telefono  
from cliente c
where c.nombre_contacto
like '%a'
...
--que tenga b en algun lugar %b%
select c.nombre_contacto, c.apellido_contacto, c.telefono  
from cliente c
where c.nombre_contacto
like '%b%'

--limite de menor o mayor<> donde nombre de conctacto sea =??? y el credito sea menos a cierto monto
select c.nombre_contacto
from cliente c
where c.nombre_contacto like 'A%'
and c.limite_credito > 1000;

--operador logico and, buscar una ciudad que tenga entre su nombre lo
select * from cliente c
where c.limite_credito>3000
and c.ciudad like '%lo%'

--operador logico or y que la ciudad sea Miami
select * from cliente c
where c.limite_credito>3000
or c.ciudad='Miami'
...
--Espacio en null
select * from cliente c
where c.linea_direccion2 is null
or c.region is null

select c.nombre_cliente, linea_direccion2 from cliente c
where c.linea_direccion2 is null
or c.region is null

--seleccione todos los clientes donde la ciudad no es San Francisco
select * from cliente c
where not ciudad='San Francisco'

--seleccione todo lo de cliente donde la ciudad IN(Miami y madrid) y el codigo de 
--empleado de representante de ventas estre entre between 5 y 8 y el nombre de cliente empiece en D
select * from cliente c
where c.ciudad in('miami','madrid')
and c.codigo_empleado_rep_ventas between 5 and 8
and c.nombre_cliente like 'd%'

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Devuelve la cantidad de clientes como total_clientes
Select count(*) as total_clientes
from cliente

--Devuelve la cantidad de clientes que tienen limite credito como clientes con credito
Select count(c.limite_credito) as clientes_con_credito
from cliente c

--Devuelve la suma de todos los limies de credito como total credito
Select sum(c.limite_credito)
as total_credito
from cliente c

--Devuelve el promedio del limite de credito de la tabla clientes
select avg(c.limite_credito) 
as promedio_cliente
from cliente c

--Devuelve el minimo del limite de credito de la tabla clientes
select min(c.limite_credito) 
as minimo_de_credito
from cliente c

--Devuelve el maximo del limite de credito de la tabla clientes
select max(c.limite_credito) 
as minimo_de_credito
from cliente c

--tabla que devuelve el credito maximo, credito minimo, promedio de credito y el total de credito 
select 
max(c.limite_credito) as credito_maximo, 
min(c.limite_credito) as credito_minimo,
avg(c.limite_credito) as promedio_credito,
sum(c.limite_credito) as total_credito,
count(*) as total_clientes
from cliente c

--Devuelve la fecha minima  y la fecha maxima de la tabla pedido y el total de pedidos
select 
...
--having sirve para filtrar datos
--group by se utiliza para agrupar filas
...
--promedio de precio de productos por gama(solo por gamas caras)
Select 
avg(p.precio_venta) as promedio_precio, p.gama
from producto p
group by p.gama
having avg(p.precio_venta)>20
...
--Me complique de mas haciendo JOIN
select o.codigo_oficina ,count(*) as cantidad_empleados
from oficina o 
...
select dp.codigo_pedido,sum(dp.cantidad) as total_productos
from detalle_pedido dp
group by codigo_pedido
having sum(dp.cantidad) > 100;

--clientes por pas]is(solo paises con muchos clientes)
select* from cliente
...
--inner join
select c.nombre_cliente,p.codigo_cliente,p.fecha_pedido 
from cliente c 
inner join pedido p
on c.codigo_cliente=p.codigo_cliente

--inner join codigo cliente en cliente y pedido
--inner join pedido y detalle pedido
--inner de producto y detalle pedido
...
--nuscar registros con valores null
select c.nombre_cliente, c.ciudad, c.region 
from cliente c
where c.region is null

select e.nombre, e.apellido1, e.codigo_jefe 
from empleado e
where e.codigo_jefe is null
...
--Clientes que si tengan limite de credito
select c.nombre_cliente, c.limite_credito
from cliente c
where c.limite_credito is not null

--oficinas que tienen una segunda direccion
select o.ciudad, o.linea_direccion2
from oficina o
where o.linea_direccion2 is not null

--remplazar valores null con COALESCE
--COALESCE sirve para mostrar otr valor cuando aparece null
select c.nombre_cliente, 
coalesce(region, 'Sin Region') as region
from cliente c

--mostrar "no tienen jefe" cuando el empleado no tenga jefe
-- cast sirve para que que un tipo de valor se tome como otro
select e.nombre, 
coalesce(cast(e.codigo_jefe as varchar), 'No tiene jefe') as jefe
from empleado e

--mostrar comentarios de pedidos, si el pedido no tiene comentarios, aparece "Sin comentarios"
select p.codigo_pedido, p.fecha_entrega , 
coalesce(p.comentarios,'Sin Comentarios') as comentarios
from pedido p

--Clientes que tienen region y limite de credito y que el limite de credito no sea null 
select c.nombre_cliente, c.region, c.limite_credito
from cliente c
...
create view vista_productos_premium 
as
select p.codigo_producto ,p.nombre, p.precio_venta 
from producto p
where precio_venta>100

select * from vista_productos_premium

--Vista con inner join
create view vista_clientes_con_pedidos 
as
select c.nombre_cliente,p.codigo_cliente,p.fecha_pedido 
from cliente c 
inner join pedido p
...
select * from vista_clientes_con_pedidos

--vista con funciones de agregacion group by

create view total_pagos_cliente
as
select p.codigo_cliente,sum(p.total) as  total_pagado
from pago p
group by p.codigo_cliente
